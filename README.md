...
**Status:** **slice 1** — zero-LLM OPNsense detect → block → receipt (+ expiry + local notify queue). See [`docs/slice-1-zero-llm.md`](docs/slice-1-zero-llm.md).  
**License:** [MIT](LICENSE)  
...
`0 → 1 → 2 → 4 → 5` then package. **Slice 0 done. Slice 1 in this tree.**
...
1. Zero-LLM OPNsense loop + `fyber.receipt/v0` + expiry + notify queue stub — [`docs/slice-1-zero-llm.md`](docs/slice-1-zero-llm.md)  
