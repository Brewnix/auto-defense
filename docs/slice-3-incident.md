...
| D | Grants: side index `grant_ids` + `flags.grant_active`. Refuse close while active. Slice 7 wires this from the grant cache. Acceptance #4 = unit reject helper. |
...
`flags.grant_active` or a grant `active_until` in the future blocks human close and auto-close. Helpers: `attach_grant`, `set_grant_active` (slice 7 `poll_grants` / `mint_local`). `require_grant_incident_id` rejects a grant body with missing / empty / null `incident_id`. Mint UX: [`docs/slice-7-privilege-grant.md`](slice-7-privilege-grant.md).
